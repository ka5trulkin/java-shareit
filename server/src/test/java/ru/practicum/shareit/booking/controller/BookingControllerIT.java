...
class BookingControllerIT extends AbstractTest {
    ...
    private final String urlPathId = String.join("/", BOOKINGS_PREFIX, "{bookingId}");
    private final String urlPathOwner = String.join("/", BOOKINGS_PREFIX, "owner");
    ...
    void getBookingsByBooker() {
        ...
        mockMvc.perform(get(BOOKINGS_PREFIX)
                        .header(X_SHARER_USER_ID, idOne)
                        .param("state", "all")
                        .param("from", "0")
                        .param("size", "1"))
